...
" PL/SQL
au BufNewFile,BufRead *.pls,*.plsql,*.pkg,*.pkb,*.bdy,*.pkh,*.pks,*.typ,*.tyb,*.tst		setf sql
...
" If global filetype variable is set, use that (e.g. 'setf sqloracle'), else use generic sql filetype.
" However, ftplugin/sql.vim already provides some SQL dialect cleverness (which I don't understand, and takes some setup, which I don't understand either)
" Looks like I need to read the help text (:help sql.txt)
func! s:SQL()
